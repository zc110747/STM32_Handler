...
static BaseType_t is_usart_driver_init = pdFALSE;
...
BaseType_t usart_driver_init(void)
{
    ...
    is_usart_driver_init = pdTRUE;

    #if RUN_TEST_MODE == USART_TEST
    printf("usart test for polling!\r\n");
    usart_run_test();
    #endif
...
void usart_translate(char *ptr, uint16_t size)
{
    ...
    //only send after init.
    if(is_usart_driver_init == pdFALSE)
        return;
